...
func TestMergeBasic(t *testing.T) {
	...
	m, ok := s.(*MergeStmt)
	if !ok {
		t.Fatalf("expected MergeStmt, got %T", s)
	}
	...
	wc0 := m.WhenClauses[0]
	if wc0.MatchKind != MERGE_WHEN_MATCHED {
		t.Errorf("expected MATCHED, got %d", wc0.MatchKind)
	}
	if wc0.CommandType != MERGE_CMD_UPDATE {
	...
	wc1 := m.WhenClauses[1]
	if wc1.MatchKind != MERGE_WHEN_NOT_MATCHED_BY_TARGET {
		t.Errorf("expected NOT_MATCHED_BY_TARGET, got %d", wc1.MatchKind)
	}
	if wc1.CommandType != MERGE_CMD_INSERT {
...
func TestMergeDelete(t *testing.T) {
	...
	s := parseOne(t, sql)
	m := s.(*MergeStmt)
	if m.WhenClauses[0].CommandType != MERGE_CMD_DELETE {
...
func TestMergeDoNothing(t *testing.T) {
	...
	s := parseOne(t, sql)
	m := s.(*MergeStmt)
	if m.WhenClauses[0].CommandType != MERGE_CMD_NOTHING {
...
func TestMergeNotMatchedBySource(t *testing.T) {
	...
	s := parseOne(t, sql)
	m := s.(*MergeStmt)
	if m.WhenClauses[0].MatchKind != MERGE_WHEN_NOT_MATCHED_BY_SOURCE {
...
func TestMergeNotMatchedByTarget(t *testing.T) {
	...
	s := parseOne(t, sql)
	m := s.(*MergeStmt)
	if m.WhenClauses[0].MatchKind != MERGE_WHEN_NOT_MATCHED_BY_TARGET {
...
func TestMergeMultipleActions(t *testing.T) {
	...
	if m.WhenClauses[1].CommandType != MERGE_CMD_UPDATE {
		t.Error("expected second clause to be UPDATE")
	}
	if m.WhenClauses[2].CommandType != MERGE_CMD_INSERT {
