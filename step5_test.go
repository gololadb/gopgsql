...
func TestWindowFrameRowsUnbounded(t *testing.T) {
	...
	if w.FrameOptions&FRAMEOPTION_NONDEFAULT == 0 {
	...
	if w.FrameOptions&FRAMEOPTION_END_CURRENT_ROW == 0 {
...
func TestWindowFrameRangeCurrentRow(t *testing.T) {
	...
	fc := sel.TargetList[0].Val.(*FuncCall)
	w := fc.Over
	if w.FrameOptions&FRAMEOPTION_RANGE == 0 {
		t.Error("expected RANGE mode")
	}
	if w.FrameOptions&FRAMEOPTION_START_CURRENT_ROW == 0 {
...
func TestWindowFrameRowsBetween(t *testing.T) {
	...
	if w.FrameOptions&FRAMEOPTION_START_OFFSET_PRECEDING == 0 {
		t.Error("expected START_OFFSET_PRECEDING")
	}
	if w.FrameOptions&FRAMEOPTION_END_OFFSET_FOLLOWING == 0 {
...
func TestWindowFrameGroupsBetweenUnbounded(t *testing.T) {
	...
	fc := sel.TargetList[0].Val.(*FuncCall)
	w := fc.Over
	if w.FrameOptions&FRAMEOPTION_GROUPS == 0 {
	...
	if w.FrameOptions&FRAMEOPTION_END_UNBOUNDED_FOLLOWING == 0 {
...
func TestWindowFrameExcludeGroup(t *testing.T) {
	...
	fc := sel.TargetList[0].Val.(*FuncCall)
	w := fc.Over
	if w.FrameOptions&FRAMEOPTION_EXCLUDE_GROUP == 0 {
...
func TestWindowFrameExcludeTies(t *testing.T) {
	...
	fc := sel.TargetList[0].Val.(*FuncCall)
	w := fc.Over
	if w.FrameOptions&FRAMEOPTION_EXCLUDE_TIES == 0 {
...
func TestWindowFrameDefault(t *testing.T) {
	...
	fc := sel.TargetList[0].Val.(*FuncCall)
	w := fc.Over
	if w.FrameOptions != FRAMEOPTION_DEFAULTS {
		t.Errorf("expected FRAMEOPTION_DEFAULTS (%d), got %d", FRAMEOPTION_DEFAULTS, w.FrameOptions)
	}
}
...
func TestWindowFrameGroupsOffsetBoth(t *testing.T) {
	...
	if w.StartOffset.(*A_Const).Val.Ival != 2 {
		t.Error("expected start offset 2")
	}
	if w.EndOffset.(*A_Const).Val.Ival != 2 {
